def get_system_prompt(maxPoiNum, numMustSee, numCandidates):
    ...
    ### Key Constraints
    - Select exactly 6 to 10 points from the provided list of {numCandidates} candidate points.
...
def get_start_point_prompt(candidate_points, user_reqs, return_candidates, distance_string):
    ...
    2. If no starting point is specified, follow these rules:
       - Ensure the selected point meets the user’s requirements: {user_reqs}
...
def get_dayplan_prompt(context_string, must_see_string, keyword_reqs, userReqList, maxPoiNum, numMustSee, numCandidates,
                       itinerary_reqs, start_end, comments="", hours=None, mark_citywalk=False):
    ...
    - **Itinerary Time**: Less than {hours} hours. Assume each point takes approximately 30-45 minutes to visit, including travel time between points. Ensure the total time for all selected points does not exceed {hours} hours. For example, 6 points take 180-270 minutes (3-4.5 hours).
    - **Point Selection**: Must follow the provided sequence order of points.
    - **Point Limit**: Select exactly 6 to 10 points, ensuring the total visit duration fits within {hours} hours. Do not select more than 10 points or fewer than 6 points.
    ...
    - **Starting and Ending Points**: The itinerary must start at the specified Starting Point ({start_poi[18:] if start_poi else "None"}) and end at the specified Ending Point ({end_poi[16:] if end_poi else "None"}). Do not alter the starting or ending points. Ensure the starting point is the first in the itinerary and the ending point is the last.
    ...
    10. Strictly meet each **Keyword Requirement**, ensuring that the itinerary includes at least one point for each keyword in {keyword_reqs}.
    11. Ensure the itinerary starts at the **Starting Point** ({start_poi[18:] if start_poi else "None"}) and ends at the **Ending Point** ({end_poi[16:] if end_poi else "None"}).
    12. Select points to strictly meet the time constraint of {hours} hours, choosing exactly 6 to 10 points that can realistically be visited within this time, with each point taking 30-45 minutes.
